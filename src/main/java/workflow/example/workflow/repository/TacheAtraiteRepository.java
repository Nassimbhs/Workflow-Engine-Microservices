...
public interface TacheAtraiteRepository extends JpaRepository<TacheAtraiter,Long> {
    ...
    @Query("SELECT t FROM TacheAtraiter t WHERE t.statut = 'non traité' AND t.responsable = :responsableId")
    List<TacheAtraiter> getTacheNontraiterByResponsable(@Param("responsableId") Long responsableId);
